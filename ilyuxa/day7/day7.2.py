...
def main():
    ...
    while True:
        time_now += 1
        for i in range(len(len_worker)):
            if len_worker[i][0] == 0 and list_statement != []:
                ...
                if set(list_class_point[point_now].list_point_before).issubset(answer) or answer == '':
                    len_worker[i][0] = point_now
                else:
                    unclaimed.append(point_now)
            elif len_worker[i][0] != 0:
                len_worker[i][1] += 1
                if len_worker[i][1] == time_point[len_worker[i][0]]:
                    answer += len_worker[i][0]
                    list_statement = list_statement + list_class_point[len_worker[i][0]].list_point_after + unclaimed
                    unclaimed = []
                    list_statement = list_st(list_statement)
                    len_worker[i] = [0, 0]
                    if list_statement!=[]:
                        if set(list_class_point[list_statement[0]].list_point_before).issubset(answer):
                            len_worker[i][0] = list_statement.pop(0)
        for i in range(len(len_worker)):
            if len_worker[i][0] == 0 and list_statement != []:
                list_statement = list_st(list_statement)
                if set(list_class_point[list_statement[0]].list_point_before).issubset(answer):
                    ...
                    unclaimed.append(list_statement.pop(0))
        for i in range(len(len_worker)):
            if len_worker[i][0] == 0 and list_statement != []:
                list_statement = list_st(list_statement)
                if set(list_class_point[list_statement[0]].list_point_before).issubset(answer):
